...
| **No APA writes at all.** Refused: format, partition creation (including a stray `open(O_CREAT)` on `hdd0:`), partition delete and rename, sub-partition add/remove, OSD MBR changes, partition swaps, and raw sector writes (`HDIOC_WRITESECTOR`). The HDD page offers no Delete, and no Rename for PS2 games; a one-game `PP.*` PS1 install says it can't be renamed. Loose VCDs and Ember folders still rename, because that is a PFS write inside their partition. | `modules/hdd/apa/src/hdd_fio.c`, `src/hddsupport.c` |
