...
>
> So are two more site files, by `.github/scripts/site_sync.py`, which docs-sync runs after every
> `rebuild/main` push and once a day: the body of `retroachievements.html` is rendered from
> `docs/RETROACHIEVEMENTS.md` (edit the Markdown), and `data/search-index.json` is rebuilt from the
> pages (edit the pages; a search entry's title and category are curated in the index and kept). To
> preview or check locally, with gh-pages checked out beside a rebuild/main checkout and
> `beautifulsoup4` + `markdown-it-py` installed:
