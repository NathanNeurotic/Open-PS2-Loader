...
// A progress callback that returns non-zero ABANDONS the wait and yields SYS_DISC_CANCELLED, so a
// slow or unresponsive drive can always be escaped by the user rather than only by the budget.
