...
# come from there at all. The header identifies local replacements explicitly.
#
...
mkdir -p rolling
{
  ...
  echo "#"
  echo "# SOME MODULES BELOW ARE NOT WHAT THE LOADER EMBEDS. The fork builds its own and"
  ...
  echo "#"
  echo "# This is an SDK inventory, not proof that every listed module is embedded."
  echo "# The Makefile selects the actual inputs; local storage hashes are appended below."
  echo "#"
  ...
  for module in modules/hdd/atad/ata_bd.irx modules/bdm/bdm.irx modules/hdd/apa/ps2hdd-osd.irx; do
    if [ -f "$module" ]; then
      ...
      echo "# Local module unavailable at manifest time: $module"
    fi
  done
} > "$OUT"
