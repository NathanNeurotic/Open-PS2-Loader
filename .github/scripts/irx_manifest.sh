...
mkdir -p rolling
{
  ...
  echo "#"
  echo "# THREE OF THE MODULES BELOW ARE NOT WHAT THE LOADER EMBEDS. The fork builds its own and"
  ...
  echo "#"
  echo "#   usbd_mini.irx   every build          -> modules/usb/usbd-ra"
  echo "#                                           (ps2sdk iop/usb/usbd @314d87e7, pre-2024-09-04)"
  echo "#   ps2ips.irx      every build          -> modules/network/ps2ips"
  echo "#                                           (ps2sdk master + four fixes marked \"RA fix\")"
  echo "#   smbman.irx      RETROACHIEVEMENTS=1  -> modules/network/smbman-ra"
  echo "#                                           (ps2sdk master + the held-image fix)"
  echo "#"
