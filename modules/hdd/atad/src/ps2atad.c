...
static int ata_init_devices(ata_devinfo_t *devinfo)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!g_ata_bd_connected[i]) { /* FORK: connect at most once per unit across re-probes */
            ...
            g_ata_bd_connected[i] = 1;
        }
#endif
    }
    /* FORK: a drive that answered the probe but failed IDENTIFY (spin-up race) has exists==0 --
       reporting 0 here would let sceAtaInit latch a dead devinfo forever. */
    return atad_devinfo[0].exists ? 0 : ATA_RES_ERR_NODEV;
}
...
ata_devinfo_t *sceAtaInit(int device)
{
    if (!ata_devinfo_init) {
        /* FORK (RiptOPL): latch init-done only on SUCCESS. Stock atad latches before probing, so a
           drive that raced spin-up at _start()'s probe stays "dead" all session and every later
           sceAtaInit (the EE retry loop, xhdd's re-probe devctl) is a no-op returning the corpse.
           Retry-after-failure is BDM-safe: every failing return in ata_init_devices() precedes its
           bdm_connect_bd() calls, and the g_ata_bd_connected guard makes connect idempotent. */
