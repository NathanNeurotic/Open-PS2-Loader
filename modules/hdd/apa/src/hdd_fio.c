...
// Validate before translating a partition-relative sector to the ATA LBA.
// ATA sees only the translated address and cannot detect an earlier wrap.
static int fioPartitionRange(s32 device, const apa_sub_t *part, u32 sector, u32 count)
{
    u32 capacity;

    if (device < 0 || device >= BLKIO_MAX_VOLUMES)
        return -ENODEV;
    capacity = hddDevices[device].totalLBA;
    if (part->start >= capacity || part->length > capacity - part->start ||
        sector > part->length || count > part->length - sector)
        return -ENXIO;
    return 0;
}
...
static int fioDataTransfer(iomanX_iop_file_t *f, void *buf, int size, int mode)
{
    ...
    if ((u32)size > 0x1FF8 - fileSlot->post)
        size = 0x1FF8 - fileSlot->post;
    if (size == 0) {
        rv = 0;
        goto finish;
    }
    rv = fioPartitionRange(f->unit, &fileSlot->parts[0], fileSlot->post + 8, size);
    if (rv < 0)
        goto finish;
    if (blkIoDmaTransfer(f->unit, buf, fileSlot->parts[0].start + fileSlot->post + 8, size, mode)) {
        rv = -EIO;
        goto finish;
    }
    fileSlot->post += size;
...
static int ioctl2Transfer(s32 device, hdd_file_slot_t *fileSlot, hddIoctl2Transfer_t *arg)
{
    ...
    if (fileSlot->nsub > APA_MAXSUB || arg->sub > fileSlot->nsub)
        return -ENODEV;

    // Preserve the APA header/reserved-area restrictions for both directions.
    if (arg->sub == 0 && arg->sector < 0x2000)
        return -EINVAL;
    if (arg->sub != 0 && arg->sector < 2)
        return -EINVAL;
    if (arg->mode != BLKIO_DIR_READ && arg->mode != BLKIO_DIR_WRITE)
        return -EINVAL;

    rv = fioPartitionRange(device, &fileSlot->parts[arg->sub], arg->sector, arg->size);
    if (rv < 0)
        return rv;
    if (arg->size == 0)
        return 0;
              ...
              void *bufp, unsigned int buflen)
{
    ...
    switch (req) {
        ...
        case HIOCGETSIZE:
            if (fileSlot->nsub > APA_MAXSUB || *(u32 *)argp > fileSlot->nsub)
                rv = -ENODEV;
            else
