...
void part_set_parent(int index, struct block_device *bd)
{
    g_part[index].bd = bd;
    g_part_bd[index] = *bd;
    g_part_bd[index].priv = &g_part[index];
    g_part_bd[index].read = part_read;
    g_part_bd[index].write = part_write;
    g_part_bd[index].flush = part_flush;
    g_part_bd[index].stop = part_stop;
}
