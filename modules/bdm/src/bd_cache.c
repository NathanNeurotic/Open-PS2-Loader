...
static int _read(struct block_device *bd, u64 sector, void *buffer, u16 count)
{
    ...
    // The cache stores 512-byte sectors and refills eight at a time. A valid
    // request near the end of the device must not become an out-of-range refill.
    if (bd->sectorSize != 512 || count >= SECTORS_PER_BLOCK ||
        SECTORS_PER_BLOCK > bd->sectorCount - sector) {
        // Do a direct read
    ...
    // Fill the block
    // Invalidate before refilling: a failed/short read may already have changed
    // part of the buffer. Never publish it under either the old or the new LBA.
    c->sector[blkidx_best] = 0xffffffffffffffff;
    int result = c->bd->read(c->bd, sector, c->cache[blkidx_best], SECTORS_PER_BLOCK);
...
struct block_device *bd_cache_create(struct block_device *bd)
{
    ...
    // Copy the installed SDK's complete layout, including optional appended
    // fields, without requiring the newer block_device.path member.
    *cbd = *bd;
    cbd->priv = c;
