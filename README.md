...
2. Copy `ART/` to the device's artwork directory. For Neutrino, copy the complete `neutrino/` folder, not only `neutrino.elf`. **No path setup is required for the normal layout:** while **Settings → Game Launching → Neutrino Defaults → Neutrino Path** shows `<not set>`, RiptOPL searches the active game's own device first and then falls back to `mc0:` / `mc1:`. That means an MMCE game naturally prefers `mmceN:/neutrino/` (or its MMCE Prefix Path) before memory-card fallback. If Neutrino lives somewhere else — for example an HDD game using Neutrino from MMCE — enter the **full `neutrino.elf` path** there; a configured path is authoritative and skips automatic searching. Examples: `mass0:/neutrino/neutrino.elf`, `mmce0:/neutrino/neutrino.elf`, `mc0:/neutrino/neutrino.elf`. Install the `POPS/` and `EMBER/` companions for PS1 as described in [PS1 games](#ps1-games-two-cores-one-list). The PSU contains the app files only.
