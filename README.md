...
2. Copy `ART/` to the device's artwork directory. For Neutrino, copy the complete `neutrino/` folder to a location the resolver can keep available through launch. On ordinary non-MMCE setups, `mc0:/neutrino/` or `mc1:/neutrino/` is supported. **For MMCE games, put Neutrino on the MMCE mass-storage device instead** — for example `mmce0:/neutrino/` (or under your configured MMCE Prefix Path). If GameID switching is enabled, an `mcN:`-hosted Neutrino would be displaced by that slot's card switch, so RiptOPL protects the launch by skipping the switch; an `mmceN:` copy is unaffected and preserves per-game card switching. **Auto** probes the active game device before the memory-card fallback. Install the `POPS/` and `EMBER/` companions for PS1 as described in [PS1 games](#ps1-games-two-cores-one-list). The PSU contains the app files only.
...
- **RetroAchievements:** a separate development package; the console integration has not been hardware-tested. It requires [xeRAbora v0.1.0-alpha.10](https://github.com/hacan359/xerabora/releases/tag/v0.1.0-alpha.10) on a PC and OPL-core launching. The RA build includes main-menu actions to check and launch a physical PS2 disc with achievements; this path also needs hardware validation. Keep games you play with achievements on a local device — upstream reports on hardware that a game with an achievement set stops loading about a minute in from a network share. [RA guide](docs/RETROACHIEVEMENTS.md).
...
- **[Neutrino](https://github.com/rickgaiser/neutrino)** by **[rickgaiser](https://github.com/rickgaiser)** — a *"Small, Fast and Modular PS2 Device Emulator"*, and RiptOPL's **second PS2 loader core** alongside OPL's own. Like Ember it is not a shortcut: a ready-to-use `neutrino/` folder ships **inside** each installable ZIP. Memory-card installs are supported for ordinary launches, but **MMCE users who use GameID card switching should place the complete folder on the MMCE game device** (`mmceN:/neutrino/`, or under the configured MMCE Prefix Path) so the loader remains independent of the switched `mcN:` view. See [docs/NEUTRINO.md](docs/NEUTRINO.md). Neutrino is deliberately **UI-agnostic** — it has no interface of its own, which is exactly what lets a front-end like RiptOPL drive it per game. Licensed **AFL-3.0**; releases: <https://github.com/rickgaiser/neutrino/releases>.
...
- **hacan359 (yoba)** — for **RetroAchievements on real PS2 hardware** and the
  **[xeRAbora PC client (v0.1.0-alpha.10)](https://github.com/hacan359/xerabora/releases/tag/v0.1.0-alpha.10)** (MIT). The console identifies the
